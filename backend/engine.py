...
# --- FINAL, MASTER, PRODUCTION-GRADE ENGINE ---
...
def run_vqe_calculation(molecule_name: str, bond_length: float, basis: str, backend_name: str):
    ...
    try:
        ...
        # We create a Session to manage the connection to the chosen backend.
        # This is the professional Qiskit workflow.
        with backend.open_session() as session:
            estimator = Estimator(session=session)
            logging.info(f"Estimator session opened for backend: {backend.name}")
            ...
            # The classical solver does not need a session
            classical_solver = NumPyMinimumEigensolver()
            classical_result = classical_solver.compute_minimum_eigenvalue(qubit_op)
            
            # This job is now managed by the session and sent to the correct backend.
